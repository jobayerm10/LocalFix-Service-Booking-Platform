        ...
        <div className="hero-content flex flex-col lg:flex-row items-center justify-between gap-2 min-h-[70vh]">
          ...
          <div className="left-content flex-1 text-white">
            ...
            <h1 className="text-xl font-[font1] lg:text-6xl font-black mb-2 leading-tight">
              Find Trusted
              <br />
              Local Services<span className="text-yellow-400">.</span>
            </h1>

            {/* Description */}
            <p className="text-md text-gray-200 mb-2 leading-relaxed max-w-lg">
              Connect instantly with skilled electricians, plumbers, tutors,
              cleaners and more — vetted professionals ready to help.
            </p>

            {/* Stats */}
            <div className="reviews flex gap-12 sm:gap-8">
              <div>
                <p className="text-4xl sm:text-3xl font-[font1] font-bold text-white">
          ...
          <div className="right-content flex-1 w-full lg:max-w-xl">
            <div className="card bg-white rounded-3xl shadow-2xl p-6 mt-4  ">
              {/* Card Title */}
              <h2 className="text-2xl font-[font2]  font-medium text-gray-800 mb-2">
                Find a Service
              </h2>
              <p className="text-gray-500 text-sm mb-4">
                Search by category, location or name
              </p>

              {/* Category Select */}
              <div className="mb-6">
                <label className="text-xs font-bold text-gray-600 uppercase tracking-wide mb-3 block">
                  Service Category
                </label>
                <select
                  value={category}
                  onChange={(e) => setCategory(e.target.value)}
                  className="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-50 text-gray-700 font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 transition cursor-pointer"
                >
                ...
                <div className="flex flex-wrap gap-2">
                  {categories.map((cat) => (
                    <Link
                      ...
                      className={`px-4 py-2 rounded-full text-sm font-semibold ${cat.color} hover:shadow-md transition`}
                    >
                      {cat.name}
                    </Link>
                  ))}
